...
//     }
// }
export class ImgScrollView extends Component {
    static propTypes = {
    ...
    render() {
        setStyle();
        let ImageList = this.getImgList();
        // console.log("背景ImageList");
                   ...
                   // minimumZoomScale={1.0}  // 子组件(图片)缩小倍数
                    ...
                    }}>
                    {
                        ImageList.map((item, index) => {
                            console.log("itemimg")
                            return (
                                <TouchableWithoutFeedback key={item.SysNo} onPress={(p) => {
                                    if(this.props.onPress!=null){
                                        this.props.onPress(p);
                                    }
                                }}>
                                    <View style={styles.bgimg}  >
                                        <BackGroundItem ref={"imgItem" + index} imgInfo={ item } pageInfo = {{index:index,size:ImageList.length}}></BackGroundItem>
                                    </View>
                                </TouchableWithoutFeedback>
                            )
                        })
...
export class BackGroundItem extends Component {
    ...
    render() {
        ...
        if (img.DefaultImageRealUri!=null&&img.DefaultImageRealUri!='') {
            bimg = img.DefaultImageRealUri;
        }
        return (
            <ImageBackground style={styles.bgimg} source={bimg} >
